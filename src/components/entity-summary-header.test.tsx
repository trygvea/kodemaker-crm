...
describe("ContactHeader", () => {
  ...
  it("renders contact info with edit button and role", () => {
    ...
    expect(screen.getByRole("link", { name: "Endre" }).getAttribute("href")).toBe(
      "/contacts/1/edit"
    );
    expect(screen.getByText("kolbjorn@example.com", { exact: false })).toBeDefined();
    expect(screen.getByText("second@example.com", { exact: false })).toBeDefined();
...
describe("CompanyHeader", () => {
  ...
  it("renders company info with edit button", () => {
    render(<CompanyHeader company={company} editHref="/customers/1/edit" />);
