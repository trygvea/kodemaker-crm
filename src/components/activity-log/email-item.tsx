      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="flex-1 min-w-0">
                <span className="text-xs text-muted-foreground">
                  {formatDateTimeWithoutSeconds(email.createdAt)}
                  {email.sourceUser && (
                    <>
                      {" "}
                      · Avsender: {email.sourceUser.firstName ?? ""}{" "}
                  ...
                  {email.recipientContact && (
                    <>
                      {" "}
                      · Mottaker: {email.recipientContact.firstName ?? ""}{" "}
          ...
          <Accordion type="single" collapsible>
            <AccordionItem value={`email-${email.id}`} className="border-none">
              <AccordionTrigger className="group hover:no-underline text-left font-normal py-0 h-auto">
                <div className="flex-1 min-w-0">
                  {email.subject && <div className="font-medium mb-1 text-sm">{email.subject}</div>}
                  <div
              ...
              <AccordionContent className="pt-0">
                <div className="whitespace-pre-wrap break-all text-sm">{email.content}</div>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        </div>
