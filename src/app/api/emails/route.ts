...
function getContentFromForwarded(
  mail: z.infer<typeof postmarkInboundSchema>
): string | undefined {
  const forwarded = parseForwardedMessages(
    mail.TextBody || mail.HtmlBody || ""
  );
  logger.info(
    { route: "/api/emails", method: "POST", mail, forwarded },
    "Parsed forwarded email"
  );
  if (forwarded && forwarded.length > 0) {
    return [
      forwarded[0].headers.subject &&
        `Subject: ${forwarded[0].headers.subject}`,
...
export async function POST(req: NextRequest) {
  ...
  const mode: MODE =
    parsed.data.Bcc && parsed.data.Bcc.trim().length > 0 ? "BCC" : "FORWARDED";

  const content =
    mode === "BCC"
      ? parsed.data.StrippedTextReply ||
  ...
  if (!createdByEmail) {
    logger.error(
      { route: "/api/emails", method: "POST" },
      "No created by email found"
    );
    return NextResponse.json(
      { error: "No created by email found" },
      { status: 400 }
    );
  }
